...
{
    public override string ModuleName => "ShowDamage by PATTHS";
    ...
    public override void Load(bool hotReload)
    {
        ...
        if (!hotReload)
        {
            // Очистка данных при холодной загрузке
        ...
        if (message.Contains('{'))
        {
            ...
            foreach (FieldInfo field in typeof(ChatColors).GetFields())
            {
                ...
                if (message.Contains(pattern, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (value != null)
                    {
                        modifiedValue = modifiedValue.Replace(pattern, value.ToString(), StringComparison.OrdinalIgnoreCase);
                    }
                }
            }
